...
void main()
{
    ...
    vec4 hEdge = texture2D(Resource, gl_TexCoord[0].xy - offy)        * -2.0 +
    ...
    vec4 vEdge = texture2D(Resource, gl_TexCoord[0].xy - offx)        *  2.0 +
    ...
    vec4 pixel = gl_Color * texture2D(Resource, gl_TexCoord[0].xy);
